...
public static class Recursion
{
    ...
    public static void SolveMaze(List<string> results, Maze maze, int x = 0, int y = 0, List<ValueTuple<int, int>>? currPath = null)
    {
        ...
        if (maze.IsValidMove(currPath, x - 1, y)) // Move up
            SolveMaze(results, maze, x - 1, y, new List<ValueTuple<int, int>>(currPath));
        if (maze.IsValidMove(currPath, x + 1, y)) // Move down
            SolveMaze(results, maze, x + 1, y, new List<ValueTuple<int, int>>(currPath));
        if (maze.IsValidMove(currPath, x, y - 1)) // Move left
            SolveMaze(results, maze, x, y - 1, new List<ValueTuple<int, int>>(currPath));
        if (maze.IsValidMove(currPath, x, y + 1)) // Move right
            SolveMaze(results, maze, x, y + 1, new List<ValueTuple<int, int>>(currPath));
    }
}
